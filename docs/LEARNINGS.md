...
- For fast Rust Playwright parity without re-implementing browser automation internals, invoking the existing Python Playwright CLI from Rust (`uv run cowork-merge export/import-browser-state`) is an effective bridge and allows `merge --auto-export-browser-state --apply` end-to-end behavior.
